...
cat > "${CONFIG_DIR}/rebexis-templates.json" <<'JSON'
{
  ...
  "modes": {
    ...
    "vote_love": {
      "_note": "Coup de cœur communauté — salue un titre qui cartonne aux votes ENCORE. Tokens {love_artist}/{love_title}. (réécrit par Rebexis 16/07 — ton antenne). ⚠️ Phrases écrites FALLBACK-SAFE : {love_artist} retombe sur 'vous', donc il n'est utilisé qu'en complément ('chapeau à {love_artist}' → 'chapeau à vous'), JAMAIS en sujet d'un verbe conjugué ('{love_artist} monte' → 'vous monte' = cassé). ⚠️ Aucune phrase ne promet de rejouer le titre : la diffusion est décidée par la régie, pas par l'antenne.",
      "templates": [
        "Vous n'arrêtez pas de voter pour {love_title}. On a compris, on n'est pas sourds.",
        "Les ENCORE pleuvent sur {love_title}. Quelque part, quelqu'un avait raison.",
        "Chapeau à {love_artist} : le public en redemande, et le public est têtu.",
        "Vous avez voté, et vous avez voté fort. Résultat : {love_title} arrive en tête des coups de cœur.",
        "Il y a {love_title}, et il y a vos ENCORE qui s'empilent dessus. Le festival a du goût."
      ]
    }
  }
}
...
def replace_playlist(file_ids: list, playlist_id: int,
    ...
    try:
        ...
        for fid in to_remove[:200]:  # plafond haut : on AJOUTE ~45/cycle, un cap de 20 laissait la playlist GONFLER (+25/cycle → 186 titres, doublons accumules = repetitions a l'antenne, bug chef 21/08). 200 draine le backlog + suit le churn (~40/cycle en regime).
            path = id_to_path[fid]
...
# Retrait AUSSI d'un prefixe « Xxx - » en tete de titre (fichiers taggés « Artiste - Titre » vs
# jumeaux « Titre » seul => cles divergentes => meme morceau echappait a l'anti-repetition, bug chef
# 21/08, 135 chansons touchees). `^.*? - ` = jusqu'au 1er « - ». Sur (grosses cles = vrais doublons).
...
def _base_key(title: str) -> str:
    """Version Python de _SONG_KEY_SQL (titre sans parentheses NI prefixe « Artiste - », alphanumerique)."""
...
SCENE_PATH_RE = os.environ.get("SCENE_PATH_RE", r"(/music/(chill|phonk|synthwave|hard|lofi|lofi2|boost|phonk2|buvette)/|/bien-francais/|/gaiverland_[a-z0-9]+/media/)")
...
# Plancher BPM de la promo melodique : le loudness (energy) seul laissait fuiter du deep/soulful
# house MOU (≤120 BPM : Robin S, CeCe Peniston, Kings of Tomorrow…) sur le jour. Le BPM sépare
# le mou (≤120) du melodic punchy Forza (122+). Resserrage demandé par le chef (option b).
FORZA_PROMOTE_BPM = float(os.environ.get("FORZA_PROMOTE_BPM", "122"))
...
def generate_playlist(count: int = 20, mood: Optional[str] = None):
    ...
    if dj:
        ...
        pool = fresh if fresh else cand  # on GARDE l'anti-repetition tant qu'il reste du neuf (avant : `>= count`=40, jamais atteint par un artiste => repete autorisee en permanence = Robin Schulz rejoue, bug chef 25/08). Ne rejoue QUE si l'artiste est totalement epuise (tout passe en 6h).
    ...
    with conn.cursor() as cur:
        cur.execute("""
            ...
            WHERE analyzed=TRUE
              AND ( mood = ANY(%s)
                    OR (%s AND mood = 'melodique' AND energy >= %s AND genre_top1 = ANY(%s) AND bpm >= %s) )
              AND id != ALL(%s)
            ...
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    WHERE id = ANY(%s) AND analyzed=TRUE
                      ...
                      AND genre_top1 = ANY(%s)
                      AND (NOT %s OR title !~* %s)
                """, (boost_ids, candidate_moods, day_mode, FORZA_PROMOTE_ENERGY, FORZA_PROMOTE_GENRES, FORZA_PROMOTE_BPM, '%rebexis_%', SCENE_PATH_RE, GENRE_WHITELIST, day_mode, HARD_TITLE_RE))
